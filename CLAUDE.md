...
apps/benchmark    # Load-test harness (v2)
packages/shared   # zod schemas + TS types + constants
infra/docker      # docker-compose.yml (run via `pnpm infra:up`)
infra/observability  # Prometheus + Grafana configs/dashboards/alerts
docs/             # PROGRESS.md, DECISIONS.md, specs/, benchmarks/
```
...
pnpm infra:up                          # start postgres+postgis+prometheus+grafana
pnpm infra:down                        # stop the infra stack
...
- `archive/PROGRESS.md` — v1 session log (archived)
- `archive/superpowers/` — v1 design specs and implementation plans (archived)
